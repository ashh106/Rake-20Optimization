...
export default function RealMap({ scenario }: { scenario: string }) {
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      ...
      const [syRes, planRes] = await Promise.all([
        fetch("/api/stockyards"),
      ...
      const byName: Record<
        string,
        { lat: number; lon: number; avail_t: number; loading_rate_tph: number }
      > = {};
      ...
      sys.forEach((s: any) => {
        ...
        m.bindTooltip(
          `<b>${s.name}</b><br/>Available: ${s.avail_t} t<br/>Active rakes: ${Math.floor(Math.random() * 5) + 1}<br/>Loading: ${s.loading_rate_tph} tph<br/>Status: ${status}`,
        );
      });

      // Routes and moving markers per rake
      const live: LiveRake[] = plan.rakes.map((r: any, i: number) => ({
        id: r.rake_id,
        from: r.source,
        to: r.destinations[0],
        eta: r.eta,
      ...
      live.forEach((r) => {
        const a = byName[r.from.toLowerCase()] || byName["bokaro"];
        const b = byName[r.to.toLowerCase()] || byName["kolkata"];
        if (!a || !b) return;
        const line = L.polyline(
          ...
          {
            color: r.status === "delayed" ? "#DC2626" : "#00b37e",
            weight: 2,
            dashArray: "6 6",
          },
        ).addTo(routes) as Polyline;
        const train = L.circleMarker([a.lat, a.lon], {
          radius: 4,
          color: r.status === "delayed" ? "#DC2626" : "#FF7F11",
          fillOpacity: 1,
        }).addTo(routes) as Circle;
        const total = 60000;
        let t0 = performance.now();
        const animate = (now: number) => {
          const p = ((now - t0) % total) / total;
          const lat = a.lat + (b.lat - a.lat) * p;
          const lon = a.lon + (b.lon - a.lon) * p;
          train.setLatLng([lat, lon]);
          train.bindTooltip(
            `${r.id} → ${r.to}<br/>ETA: ${new Date(r.eta).toLocaleTimeString()}<br/>Cost: ₹${(r.cost || 0).toLocaleString("en-IN")}`,
          );
      ...
      <div className="absolute top-3 right-3 rounded-md border bg-background/90 backdrop-blur p-2 text-xs space-y-1 shadow">
        ...
        <label className="flex items-center gap-2">
          <input
            ...
            onChange={(e) =>
              setToggles({ ...toggles, active: e.target.checked })
            }
          />{" "}
          Active Rakes
        </label>
        <label className="flex items-center gap-2">
          <input
            type="checkbox"
            checked={toggles.capacities}
            onChange={(e) =>
              setToggles({ ...toggles, capacities: e.target.checked })
            }
          />{" "}
        ...
        <label className="flex items-center gap-2">
          <input
            ...
            onChange={(e) =>
              setToggles({ ...toggles, maintenance: e.target.checked })
            }
          />{" "}
          Maintenance
        </label>
        <label className="flex items-center gap-2">
          <input
            ...
            onChange={(e) =>
              setToggles({ ...toggles, heatmap: e.target.checked })
            }
          />{" "}
          Cost Heatmap
        </label>
